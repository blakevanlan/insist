...
Remover.prototype.removeAlias_ = function(source, alias) {
   ...
   while ((result = alias.exec(source)) !== null) {
      references.push(this.findReference_(source, result.index, result[0]));
   }
   for (var i = references.length - 1; i >= 0; i--) {
      source = source.slice(0, references[0].startIndex) + source.slice(references[0].endIndex);
   }
   return source;
};
